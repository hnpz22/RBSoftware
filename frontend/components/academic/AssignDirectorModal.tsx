...
export function AssignDirectorModal({ gradeId, onClose, onAssigned }: Props) {
  ...
  async function handleAssign(user: User) {
    ...
    setError(null)
    try {
      await academicService.assignDirector(gradeId, user.public_id)
      onAssigned()
    } catch (err: any) {
      setError(err?.detail ?? 'Error al asignar director')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-md rounded-lg border bg-card shadow-xl">
        <div className="flex items-center justify-between border-b px-5 py-4">
          <h3 className="font-semibold">Asignar director</h3>
